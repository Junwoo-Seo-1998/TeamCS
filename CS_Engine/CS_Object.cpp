...
void CS_Object::ShowGui()
{
	...
	{
		std::array<char, 50> temp_name{ 0, };
		std::copy(std::begin(m_name), std::end(m_name), std::begin(temp_name));
		ImGui::InputText("Name", temp_name.data(), temp_name.size());
		m_name = temp_name.data();
